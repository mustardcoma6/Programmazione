...
class LeagueController extends Controller
{
    ...
    public function societaIndex()
    {
        ...
        // Prendiamo tutti i partecipanti caricando i dati dell'utente (user)
        $participants = LeagueParticipant::where('league_id', $league->id)
            ->with('user') 
            ->get();

        // Calcoliamo gli anni usati per ogni partecipante
    ...
    public function store(Request $request) {
        $request->validate(['name' => 'required|string', 'initial_budget' => 'required|integer', 'team_name' => 'required|string']);
    ...
    public function joinStore(Request $request) {
        $request->validate(['invite_code' => 'required|string', 'team_name' => 'required|string']);
        
        $league = League::where('invite_code', $request->invite_code)->first();
        if (!$league) return back()->withErrors(['invite_code' => 'Codice non valido']);
    ...
    public function updateMarket(Request $request, League $league) {
        if (auth()->id() !== $league->admin_id) return back();
        $league->update([
            'market_start_at' => $request->market_start_at,
            'market_end_at' => $request->market_end_at
        ]);
        return back();
    }
}
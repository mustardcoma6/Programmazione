...
public function dashboard()
    {
        ...
        if ($firstLeague) {
            ...
            $stats = ['topPlayer' => $topSigning ? $topSigning->player->name : 'Nessuno', 'topPrice' => $topSigning ? $topSigning->purchase_price : 0, 'rank' => $rank, 'totalParticipants' => count($allParticipants)];
        }
...
// NUOVA FUNZIONE: GESTIONE FINANZE (ADMIN)
    public function manageFinances()
    {
        ...
        // Calcoliamo statistiche di lega
        $totalCreditsInPlay = $participants->sum('remaining_budget');
        $totalYearsInPlay = $participants->sum('years_budget');
        
        return Inertia::render('Admin/Finances', [
            'league' => $league,
            'participants' => $participants,
            'stats' => [
                'totalCredits' => $totalCreditsInPlay,
                'totalYears' => $totalYearsInPlay,
                'avgCredits' => count($participants) > 0 ? round($totalCreditsInPlay / count($participants)) : 0
            ]
        ]);
    }

...
class MarketController extends Controller
{
    ...
    public function sessions() {
        ...
        return Inertia::render('Market/Sessions', ['league' => League::find($participant->league_id), 'sessions' => $sessions]);
    }
    ...
    public function storeSession(Request $request) {
        ...
        MarketSession::create(['league_id' => $participant->league_id, 'start_at' => $request->start_at, 'end_at' => $request->end_at, 'auction_duration' => $tm, 'allowed_roles' => implode(',', $request->roles ?? ['P','D','C','A'])]);
        return back();
    }
    ...
    public function closeMarketNow() {
        $participant = LeagueParticipant::where('user_id', auth()->id())->first();
        if ($participant) {
            MarketSession::where('league_id', $participant->league_id)->where('end_at', '>', now())->update(['end_at' => now()]);
            Auction::where('league_id', $participant->league_id)->where('is_finished', false)->delete();
        }
        return back();
    }

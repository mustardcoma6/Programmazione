...
class MarketController extends Controller
{
    ...
    public function closeMarketNow() {
        ...
        if ($participant) {
            \App\Models\MarketSession::where('league_id', $participant->league_id)->where('end_at', '>', now())->update(['end_at' => now()]);
            \App\Models\Auction::where('league_id', $participant->league_id)->where('is_finished', false)->delete();
        }
        return redirect()->route('market.auctions');
    }

    // --- ALTRE FUNZIONI OBBLIGATORIE ---
    ...
    public function buy(Request $request) { $request->validate(['player_id' => 'required', 'price' => 'nullable|integer']); $l = League::findOrFail($request->league_id); $now = Carbon::now('Europe/Rome'); $s = MarketSession::where('league_id', $l->id)->where('start_at', '<=', $now)->where('end_at', '>=', $now)->first(); if (!$s) return back(); $a = Auction::where('league_id', $l->id)->where('real_player_id', $request->player_id)->where('is_finished', false)->first(); if (!$a) { if ($now->diffInMinutes($s->end_at, false) < 90) return back(); $a = Auction::create(['league_id' => $l->id, 'real_player_id' => $request->player_id, 'user_id' => auth()->id(), 'current_bid' => 0, 'expires_at' => $now->copy()->addMinutes($s->auction_duration), 'is_finished' => false]); } $nb = $request->price ?? ($a->current_bid + 1); $this->executeBiddingWar($a, auth()->id(), $nb); if ($now->diffInSeconds($a->expires_at, false) <= 30) $a->update(['expires_at' => $now->copy()->addMinute()]); return back(); }

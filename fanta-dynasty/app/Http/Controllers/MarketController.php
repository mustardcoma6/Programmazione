...
class MarketController extends Controller
{
    ...
    // --- SEZIONE SOCIETÀ: PRIMAVERA (AGGIORNATA CON DATI REALI) ---
    public function primaveraPage() {
        ...
        return Inertia::render('Societa/Primavera', [
            'myData' => $participant,
             aristocratic'primaveraPlayers' => $primaveraPlayers // Mandiamo la lista alla grafica
        ]);
    }

    // --- LOGICHE MERCATO E ASTE ---
    public function auctions() {
        ...
        $primaveraIds = PrimaveraRoster::where('league_id', $league->id)->pluck('real_player_id')->toArray(); // Escludiamo anche i primavera dal listone
        ...
        return Inertia::render('Market/Auctions', ['league' => $league, 'isMarketOpen' => (bool)$currentSession, 'currentSession' => $currentSession, 'myData' => $participant, 'frozenCredits' => (int)$frozenCredits, 'myRoster' => $myRoster, 'availablePlayers' => $availablePlayers, 'activeAuctions' => Auction::where('league_id', $league->id)->where('is_finished', false)->with(['player', 'user'])->get()]);
    }

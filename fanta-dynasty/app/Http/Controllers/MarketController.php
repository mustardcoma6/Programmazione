...
class MarketController extends Controller
{
    public function financesPage() {
        $user = auth()->user(); $p = LeagueParticipant::where('user_id', $user->id)->first(); if (!$p) return redirect()->route('dashboard');
        $euroValues = ['SAO PAULO' => 68.12, 'ATLETICO G MINEIRO' => 59.54, 'SANTOS' => 56.94, 'FLAMENGO' => 45.24, 'CRUZEIRO E.C.' => 40.82, 'PALMEIRAS' => 39.00, 'CORINTHIANS' => 39.00, 'VASCO DE GAMA' => 33.28, 'BOTAFOGO' => 33.02, 'FLUMINENSE' => 30.94];
        $all = LeagueParticipant::where('league_id', $p->league_id)->with('user')->get();
        foreach ($all as $t) {
            $proV = DB::table('rosters')->join('real_players', 'rosters.real_player_id', '=', 'real_players.id')->where('rosters.user_id', $t->user_id)->where('rosters.league_id', $p->league_id)->sum('real_players.quotation');
            $priV = DB::table('primavera_rosters')->join('real_players', 'primavera_rosters.real_player_id', '=', 'real_players.id')->where('primavera_rosters.user_id', $t->user_id)->where('primavera_rosters.league_id', $p->league_id)->sum('real_players.quotation');
            $t->total_quotation_value = (int)$proV + (int)$priV;
            $t->euro_value = $euroValues[strtoupper(trim($t->team_name))] ?? 0.00;
        }
        $hist = MarketValueHistory::where('user_id', $user->id)->orderBy('recorded_at', 'asc')->get(['value', 'recorded_at']);
        return Inertia::render('Societa/Finances', ['myData' => $p, 'rankingAsset' => $all->sortByDesc('total_quotation_value')->values()->all(), 'rankingEuro' => $all->sortByDesc('euro_value')->values()->all(), 'myEuroValue' => $euroValues[strtoupper(trim($p->team_name))] ?? 0.00, 'history' => $hist]);
    }

    public function buy(Request $request) {
        $l = League::findOrFail($request->league_id); $now = Carbon::now('Europe/Rome');
        $s = MarketSession::where('league_id', $l->id)->where('start_at', '<=', $now)->where('end_at', '>=', $now)->first();
        if (!$s) return back();
        $a = Auction::where('league_id', $l->id)->where('real_player_id', $request->player_id)->where('is_finished', false)->first();
        if (!$a) { $a = Auction::create(['league_id' => $l->id, 'real_player_id' => $request->player_id, 'user_id' => auth()->id(), 'current_bid' => 0, 'expires_at' => $now->copy()->addMinutes($s->auction_duration), 'is_finished' => false]); }
        if ($request->max_autobid) Autobid::updateOrCreate(['auction_id' => $a->id, 'user_id' => auth()->id()], ['max_bid' => $request->max_autobid]);
        $nb = $request->price ?? ($a->current_bid + 1); $this->executeBiddingWar($a, auth()->id(), $nb);
        if ($now->diffInSeconds($a->expires_at, false) <= 30) $a->update(['expires_at' => $now->copy()->addMinute()]);
        return back();
    }

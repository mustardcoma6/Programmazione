...
class MarketController extends Controller
{
    public function financesPage() {
        ...
        $euroValues = [
            'SAO PAULO' => 68.12, 'ATLETICO G MINEIRO' => 59.54, 'SANTOS' => 56.94, 'FLAMENGO' => 45.24,
            'CRUZEIRO E.C.' => 40.82, 'PALMEIRAS' => 39.00, 'CORINTHIANS' => 39.00, 'VASCO DE GAMA' => 33.28,
            'BOTAFOGO' => 33.02, 'FLUMINENSE' => 30.94
        ];

        $allTeams = LeagueParticipant::where('league_id', $participant->league_id)->with('user')->get();
        foreach ($allTeams as $team) {
            $proV = DB::table('rosters')->join('real_players', 'rosters.real_player_id', '=', 'real_players.id')->where('rosters.user_id', $team->user_id)->where('rosters.league_id', $participant->league_id)->sum('real_players.quotation');
            $priV = DB::table('primavera_rosters')->join('real_players', 'primavera_rosters.real_player_id', '=', 'real_players.id')->where('primavera_rosters.user_id', $team->user_id)->where('primavera_rosters.league_id', $participant->league_id)->sum('real_players.quotation');
            $team->total_quotation_value = (int)$proV + (int)$priV;
            $teamNameClean = strtoupper(trim($team->team_name));
            $team->euro_value = $euroValues[$teamNameClean] ?? 0.00;
        }

        $history = MarketValueHistory::where('user_id', $user->id)->orderBy('recorded_at', 'asc')->get(['value', 'recorded_at']);
    ...
    public function closeMarketNow() {
        ...
        if ($participant) {
            ...
            MarketSession::where('league_id', $leagueId)->where('end_at', '>', now())->update(['end_at' => now()]);
            Auction::where('league_id', $leagueId)->where('is_finished', false)->delete();
        }
        return back()->with('message', 'Aste annullate con successo.');
    }
    ...
    public function storeSession(Request $request) {
        ...
        MarketSession::create(['league_id' => $p->league_id, 'start_at' => $request->start_at, 'end_at' => $request->end_at, 'auction_duration' => $tm, 'allowed_roles' => implode(',', $request->roles ?? ['P','D','C','A'])]);
        return back();
    }

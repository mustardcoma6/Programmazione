...
class MarketController extends Controller
{
    ...
    public function auctions() {
        ...
        // --- LOGICA IMPEGNO CREDITI ---
        // Calcoliamo quanti crediti l'utente sta usando per vincere le aste attuali
        ...
        return Inertia::render('Market/Auctions', [
            ...
            'myData' => $participant,
            'frozenCredits' => (int)$frozenCredits, // Passiamo i crediti "congelati"
            'myRoster' => $myRoster,
    ...
    public function buy(Request $request) {
        ...
        // Calcolo Budget Disponibile Reale (Totale - Impegnati nelle ALTRE aste)
        $participant = LeagueParticipant::where('league_id', $league->id)->where('user_id', $user->id)->first();
        $otherFrozen = Auction::where('league_id', $league->id)
            ->where('user_id', $user->id)
            ->where('real_player_id', '!=', $request->player_id) // Escludiamo il giocatore attuale se stavamo già vincendo
            ->where('is_finished', false)
            ->sum('current_bid');
        
        $availableNow = $participant->remaining_budget - $otherFrozen;

        $bidToProcess = $request->price ?? ($auction ? $auction->current_bid + 1 : 1);
        if ($request->max_autobid) $bidToProcess = max($bidToProcess, $request->max_autobid);

        if ($bidToProcess > $availableNow) {
            return back()->withErrors(['error' => "Budget insufficiente! Hai già impegnato " . $otherFrozen . " cr in altre aste."]);
        }
    ...
    // ... [executeBiddingWar, release, updateContract, history, sessions, storeSession, closeMarketNow, processExpiredAuctions rimangono uguali] ...

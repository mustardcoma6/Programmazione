...
class MarketController extends Controller
{
    ...
    public function closeMarketNow(League $league) {
        // Chiude tutte le sessioni attive portando la fine a "ora"
        MarketSession::where('league_id', $league->id)->where('end_at', '>', now())->update(['end_at' => now()]);
        // Annulla tutte le aste non finite
        Auction::where('league_id', $league->id)->where('is_finished', false)->delete();
        return back()->with('message', 'Mercato chiuso e aste annullate.');
    }
    ...
    public function auctions() {
        ...
        return Inertia::render('Market/Auctions', ['league' => $league, 'isMarketOpen' => (bool)$currentSession, 'currentSession' => $currentSession, 'myData' => $participant, 'myRoster' => $myRoster, 'availablePlayers' => $availablePlayers, 'activeAuctions' => Auction::where('league_id', $league->id)->where('is_finished', false)->with(['player', 'user'])->get()]);
    }
    ...
    public function buy(Request $request) {
        ...
        if (!$auction) {
            ...
            $auction = Auction::create(['league_id' => $league->id, 'real_player_id' => $request->player_id, 'user_id' => auth()->id(), 'current_bid' => 0, 'expires_at' => $now->addMinutes(90), 'is_finished' => false]);
        }

...
class MarketController extends Controller
{
    ...
    public function buy(Request $request) 
    {
        return DB::transaction(function () use ($request) {
            ...
            // 1. Se l'asta esiste, assicuriamoci che l'attuale leader abbia un record in Autobid
            if ($a && $a->user_id) {
                Autobid::firstOrCreate(
                    ['auction_id' => $a->id, 'user_id' => $a->user_id],
                    ['max_bid' => $a->current_bid] // Se non ha autobid, il suo limite è il prezzo attuale
                );
            }

            // 2. Controllo Budget dell'utente che sta puntando ora
            $p = LeagueParticipant::where('league_id', $l->id)->where('user_id', $user->id)->first();
            if ($p->remaining_budget < $offertaMassimaInput) {
                return back()->withErrors(['error' => 'Budget insufficiente per questa offerta.']);
            }
            ...
            if (!$a) {
                ...
                // Se rilanci, devi offrire più del prezzo attuale
                if ($offertaMassimaInput <= $a->current_bid) {
                    return back()->withErrors(['error' => 'L\'offerta deve superare il prezzo attuale di ' . $a->current_bid]);
                }
            }

            // 3. Salva/Aggiorna l'autobid del nuovo offerente
            Autobid::updateOrCreate(
                ['auction_id' => $a->id, 'user_id' => $user->id],
                ['max_bid' => $offertaMassimaInput]
            );

            // 4. Esegui la guerra tra i limiti massimi
    ...
    {
        // Prendo i primi 2 Autobid (i più alti)
        ...
        if (!$secondo) {
            // C'è solo uno: se il prezzo era 0 va a 1, altrimenti resta dov'è
            $a->update([
            ...
            if ($vincitore->max_bid > $secondo->max_bid) {
                ...
                // Pareggio: vince il primo che ha puntato quella cifra
                $a->update([
                    'user_id' => $vincitore->user_id,
    ...
    // --- UTILITY E FINANZE (Sistemate per funzionare con i nuovi dati) ---
    public function sessions() { $u=auth()->user(); $p=LeagueParticipant::where('user_id',$u->id)->first(); $l=League::find($p->league_id); return Inertia::render('Market/Sessions',['league'=>$l,'sessions'=>MarketSession::where('league_id',$l->id)->orderBy('start_at','desc')->get()]); }
    public function storeSession(Request $request) { $p=LeagueParticipant::where('user_id',auth()->id())->first(); $i=$request->auction_time; $m=str_contains($i,':')?((int)explode(':',$i)[0]*60)+(int)explode(':',$i)[1]:(int)$i; MarketSession::create(['league_id'=>$p->league_id,'start_at'=>$request->start_at,'end_at'=>$request->end_at,'auction_duration'=>$m ?: 5,'allowed_roles'=>implode(',',$request->roles)]); return back(); }
    public function financesPage() {
        ...
        return Inertia::render('Societa/Finanze',['leagues'=>$lgs,'myData'=>$lp,'allTeams'=>$all,'history'=>MarketValueHistory::where('user_id',$u->id)->orderBy('matchday','asc')->get(),'stats'=>['valore_monetario'=>$mio['valore'],'trend'=>$mio['trend'],'asset_quality_perc'=>round((Roster::where('user_id',$u->id)->with('player')->get()->sum(fn($r)=>$r->player->quotation??0)/($bench?:1))*100,1),'winning_efficiency_perc'=>round(($calc($lp->total_points/$lp->games_played)/$maxG)*100,1),'tuoi_gol'=>$calc($lp->total_points/$lp->games_played),'leader_gol'=>$maxG,'benchmark_val'=>$bench]]);
    }

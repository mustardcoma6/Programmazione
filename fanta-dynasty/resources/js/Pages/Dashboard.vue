    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex flex-col">
                <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight italic">Benvenuto Pres!</h1>
                <p class="text-sm text-gray-500 font-medium uppercase tracking-widest mt-1">Area Direzione Tecnica</p>
            </div>
        </template>
        ...
        <div class="py-10">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
                ...
                <div v-if="leagues && leagues.length > 0" class="space-y-8">
                            ...
                            <div class="flex-1 text-center lg:text-left">
                                ...
                                <div class="mt-4 flex items-center justify-center lg:justify-start gap-3">
                                    <div class="px-4 py-1.5 rounded-full text-xs font-black uppercase flex items-center gap-2 border" 
                                         :class="isMarketActive() ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-600 border-red-200'">
                                        <span class="w-2 h-2 rounded-full" :class="isMarketActive() ? 'bg-green-500 animate-pulse' : 'bg-red-500'"></span>
                                        Mercato {{ isMarketActive() ? 'Aperto' : 'Chiuso' }}
                                    </div>
                                    <span class="text-[10px] font-mono text-gray-300 uppercase tracking-widest">ID: {{ leagues[0].invite_code }}</span>
                                </div>
                            </div>
                    ...
                    <!-- 2. NAVIGAZIONE INTUITIVA (4 TASTI) -->
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <!-- SVINCOLATI (Nuovo Tasto al posto di Campo) -->
                    ...
                    <div v-if="currentLineup" class="bg-green-700 p-8 shadow-2xl rounded-3xl border-[6px] border-green-800 text-white relative overflow-hidden">
                        ...
                        <div class="flex justify-between items-center mb-8 relative z-10">
                            <h3 class="font-black uppercase tracking-tighter text-xl">L'11 Titolare <span class="text-green-300 ml-2">{{ currentLineup.module }}</span></h3>
                            <a :href="route('lineup.index')" class="bg-green-900/50 hover:bg-green-900 px-4 py-2 rounded-xl text-xs font-bold transition border border-white/10 uppercase">
                                Modifica Campo
                            </a>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 relative z-10">
                            <div v-for="detail in currentLineup.details" :key="detail.id" 
                                 class="p-4 rounded-xl bg-white/10 border border-white/10 backdrop-blur-md text-center shadow-sm">
                ...
                <div v-else class="bg-white p-16 text-center shadow-2xl rounded-3xl border-2 border-dashed border-indigo-200">
                    <div class="max-w-md mx-auto">
                        ...
                        <h3 class="text-3xl font-black text-gray-900 uppercase tracking-tighter">Inizia il tuo Campionato!</h3>
                        <p class="text-gray-500 mt-4 mb-10 text-lg">Non sei ancora iscritto a nessuna lega. Entra subito nel vivo del gioco.</p>
                        
                        <div class="flex flex-col gap-4">
                            <a :href="route('leagues.create')" class="bg-green-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-green-700 transition shadow-lg shadow-green-200">
                                ➕ Crea una Nuova Lega
                            </a>
                            <a :href="route('leagues.join')" class="bg-orange-500 text-white py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-orange-600 transition shadow-lg shadow-orange-200">
                                🤝 Unisciti a una Lega
                            </a>
                        </div>
                    </div>

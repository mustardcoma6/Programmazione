        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div v-if="leagues && leagues.length > 0" class="space-y-6">
                    ...
                    <!-- FORMAZIONE (Solo se esiste) -->
                    <div v-if="currentLineup" class="bg-green-700 p-8 shadow-2xl rounded-2xl border-4 border-green-800 text-white">
                        <h3 class="font-black uppercase mb-6 flex justify-between items-center">
                            <span>Schieramento Domenicale</span>
                            <span class="text-xs bg-green-900/50 px-3 py-1 rounded-full">{{ currentLineup.module }}</span>
                        </h3>
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            <div v-for="detail in currentLineup.details" :key="detail.id" class="p-3 rounded-lg bg-white/10 border border-white/10 text-center">
                                <p class="text-[10px] font-black text-green-300 uppercase">{{ detail.player.role }}</p>
                                <p class="text-sm font-bold truncate uppercase">{{ detail.player.name }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-else class="bg-white p-12 text-center shadow-xl rounded-2xl border-2 border-dashed border-blue-200">
                    <div class="max-w-md mx-auto">
                        ...
                        <p class="text-gray-500 mt-2 mb-8">Non sei ancora iscritto a nessuna lega. Vuoi crearne una nuova con i tuoi amici o unirti a una esistente?</p>
                        
                        <div class="flex flex-col gap-4">
                            <a :href="route('leagues.create')" class="bg-green-600 text-white py-4 rounded-xl font-black uppercase tracking-widest hover:bg-green-700 transition text-center">
                                ➕ Crea una Nuova Lega
                            </a>
                            <a :href="route('leagues.join')" class="bg-orange-500 text-white py-4 rounded-xl font-black uppercase tracking-widest hover:bg-orange-600 transition text-center">
                                🤝 Unisciti con Codice
                            </a>
                        </div>
                    </div>

...
const form = useForm({
    classifica: props.participants.map(p => ({
        id: p.id,
        name: p.user.name,
        league_points: p.league_points || 0, // <--- Assicurati che ci sia questa riga!
...
<template>
    ...
    <div class="text-center">
        <label class="block text-[10px] font-bold text-blue-500 uppercase">Punti Class.</label>
        <input type="number" v-model="team.league_points" class="w-16 border-gray-200 rounded-lg font-bold text-center bg-blue-50">
    </div>
    <div class="text-center">
        <label class="block text-[10px] font-bold text-gray-400 uppercase">Partite</label>
        <input type="number" v-model="team.games_played" class="w-14 border-gray-200 rounded-lg font-bold text-center">
    </div>
    <div class="text-center">
        <label class="block text-[10px] font-bold text-gray-400 uppercase">Punteggio Tot.</label>

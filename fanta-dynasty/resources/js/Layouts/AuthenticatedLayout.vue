        ...
        <nav class="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    ...
                    <!-- LATO SINISTRO: LOGO + NAVIGAZIONE -->
                        ...
                        <div class="hidden sm:flex sm:items-center sm:space-x-6 h-full">
                            ...
                            <template v-if="hasLeague">
                                ...
                                <Dropdown align="left" width="48">
                                    <template #trigger>
                                        <button class="inline-flex items-center px-1 pt-1 text-sm font-bold uppercase text-gray-500 hover:text-gray-700 transition duration-150 ease-in-out h-16 border-b-2 border-transparent">
                            ...
                            <NavLink :href="route('players.index')" :active="route().current('players.index')" class="h-16 flex items-center">
                                Svincolati
                            </NavLink>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div v-if="isAdmin" class="hidden sm:flex items-center">
                            <Dropdown align="right" width="48">
                                <template #trigger>
                                    <button class="bg-red-50 text-red-600 border border-red-100 px-4 py-2 rounded-xl text-xs font-black uppercase hover:bg-red-600 hover:text-white transition shadow-sm flex items-center gap-2">
                                        ⚙️ Gestione
                                    </button>
                                </template>
                                <template #content>
            ...
            <div :class="{'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown}" class="sm:hidden bg-white border-t border-gray-100 shadow-2xl overflow-y-auto max-h-[90vh]">
                <div class="pt-2 pb-3 space-y-1">
                    ...
                    <template v-if="hasLeague">
                        ...
                        <div v-show="openSocieta" class="bg-gray-50"><ResponsiveNavLink :href="route('roster.index')">La mia Rosa</ResponsiveNavLink><ResponsiveNavLink :href="route('roster.lineup')">Formazione</ResponsiveNavLink><ResponsiveNavLink :href="route('roster.finances')">Finanze</ResponsiveNavLink><ResponsiveNavLink :href="route('roster.primavera')">Primavera</ResponsiveNavLink></div>
                        <button @click="openLega = !openLega" class="w-full text-left pl-3 pr-4 py-2 font-bold text-gray-600 uppercase flex justify-between">Lega <svg class="h-4 w-4" :class="{'rotate-180': openLega}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 9l-7 7-7-7" /></svg></button>
                        <div v-show="openLega" class="bg-gray-50"><ResponsiveNavLink :href="route('societa.index')">Anagrafe Lega</ResponsiveNavLink><ResponsiveNavLink :href="route('teams.index')">Rose Avversarie</ResponsiveNavLink><ResponsiveNavLink :href="route('league.ranking')">Ranking</ResponsiveNavLink><ResponsiveNavLink :href="route('league.trophies')">Sala Trofei</ResponsiveNavLink></div>
                        <button @click="openMercato = !openMercato" class="w-full text-left pl-3 pr-4 py-2 font-bold text-gray-600 uppercase flex justify-between">Calciomercato <svg class="h-4 w-4" :class="{'rotate-180': openMercato}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 9l-7 7-7-7" /></svg></button>
                        <div v-show="openMercato" class="bg-gray-50"><ResponsiveNavLink :href="route('market.auctions')">Aste e Scambi</ResponsiveNavLink><ResponsiveNavLink :href="route('market.history')">Cronologia</ResponsiveNavLink></div>
                    </template>
                    <div v-if="isAdmin">
                        <button @click="openGestione = !openGestione" class="w-full text-left pl-3 pr-4 py-2 font-bold text-red-600 uppercase flex justify-between">⚙️ Gestione <svg class="h-4 w-4" :class="{'rotate-180': openGestione}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M19 9l-7 7-7-7" /></svg></button>
                        <div v-show="openGestione" class="bg-red-50"><ResponsiveNavLink :href="route('market.sessions')">Sessioni</ResponsiveNavLink><ResponsiveNavLink :href="route('admin.rosters')">Rose</ResponsiveNavLink><ResponsiveNavLink :href="route('admin.credits')">Budget</ResponsiveNavLink><ResponsiveNavLink :href="route('admin.primavera')">Primavera</ResponsiveNavLink></div>
                    </div>
                </div>
            </div>
        </nav>

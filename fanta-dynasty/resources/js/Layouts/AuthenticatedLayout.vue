                            ...
                            <template v-if="hasLeague">
                                ...
                                <div class="hidden sm:flex sm:items-center">
                                    <Dropdown align="left" width="48">
                                        <template #trigger>
                                            <button class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-bold leading-5 text-gray-500 hover:text-gray-700 transition uppercase" :class="{'border-blue-500 text-gray-900': route().current('roster.*')}">
                                ...
                                <div class="hidden sm:flex sm:items-center">
                                    <Dropdown align="left" width="48">
                                        <template #trigger>
                                            <button class="inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-bold leading-5 text-gray-500 hover:text-gray-700 transition uppercase" :class="{'border-blue-500 text-gray-900': route().current('societa.*') || route().current('teams.*') || route().current('players.*') || route().current('league.*')}">
            ...
            <div :class="{'block': showingNavigationDropdown, 'hidden': !showingNavigationDropdown}" class="sm:hidden bg-white border-t border-gray-100 shadow-2xl overflow-y-auto max-h-[90vh]">
                <div class="pt-2 pb-3 space-y-1">
                    ...
                    <template v-if="hasLeague">
                        ...
                        <div v-show="openSocieta" class="bg-gray-50"><ResponsiveNavLink :href="route('roster.index')">La mia Rosa</ResponsiveNavLink><ResponsiveNavLink :href="route('roster.primavera')">Primavera</ResponsiveNavLink></div>
                        <button @click="openLega = !openLega" class="w-full text-left pl-3 pr-4 py-2 font-bold text-gray-600 uppercase">Lega</button>
                        <div v-show="openLega" class="bg-gray-50"><ResponsiveNavLink :href="route('societa.index')">Anagrafe</ResponsiveNavLink><ResponsiveNavLink :href="route('teams.index')">Rose Avversarie</ResponsiveNavLink><ResponsiveNavLink :href="route('players.index')">Svincolati</ResponsiveNavLink></div>
                    </template>
                </div>
            </div>
        </nav>
